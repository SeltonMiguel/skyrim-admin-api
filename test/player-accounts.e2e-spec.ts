...
describeDatabase('Player accounts with real PostgreSQL', () => {
  ...
  it('adds only players and player_identities, with no schema diff and no staff relation', async () => {
    ...
    // (10.13) and marketplace (10.14) idempotency records.
    expect(
      foreignKeys.sort((a: { source: string }, b: { source: string }) =>
        a.source.localeCompare(b.source),
      ),
    ).toEqual([
      { source: 'economy_transactions', target: 'players' },
      { source: 'game_commands', target: 'players' },
      { source: 'player_characters', target: 'players' },
      { source: 'player_chat_requests', target: 'players' },
      { source: 'player_identities', target: 'players' },
      { source: 'player_marketplace_requests', target: 'players' },
      { source: 'player_sessions', target: 'players' },
      { source: 'player_settings', target: 'players' },
      { source: 'player_trade_requests', target: 'players' },
      { source: 'player_vip_entitlements', target: 'players' },
    ]);
    expect(
      await database.query(
